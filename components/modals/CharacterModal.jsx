...
export default function CharacterModal({ setShowModal }) {
  const ref = useRef();
  ...
  const saveCharacter = async () => {
    ...
    const character = {
      key: characterName,
      characteristic: editorText,
    };

    const characterResponse = await fetch(
      "/api/user/character/upsertCharacter",
      {
        method: "POST",
        headers: {
  ...
  const deleteCharacter = async (char) => {
    const character = {
      key: characterName ? characterName : char.key,
    };

    console.log(character);

    const characterResponse = await fetch(
      "/api/user/character/deleteCharacter",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
              ...
              {character.map((char, index) => {
                return (
                  <div
                    ...
                    onClick={() => {
                      setEditorText(char.characteristic);
                      setCharacterName(char.key);
                    }}
                  >
                    {char.key}
                    <div
